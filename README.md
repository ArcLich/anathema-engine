...
Uses codekiddy2's [opening book](https://sourceforge.net/projects/codekiddy-chess/files/Books/Polyglot%20books/Update1/). To run, download and save as `Book.bin` in directory `Opening TB`, or turn off opening book usage by setting `OPENING` to `False` in `util.py`.
